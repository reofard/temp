...
export const deletePost = async(id:string)=>{
    try {
        const post = await Axios.delete(`http://usoptiontown.com:5001/post/${id}`)
        if(post){
  ...
  export const dislikePost = async (id: string) => {
    try {
      const { data: response } = await Axios.put(
        `http://usoptiontown.com:5001/post/dislikePost/${id}`,
...
// }
  ...
  export const addComment = async (id: string, newComment:any) => {
    ...
    try {
      const { data: response } = await Axios.put(
        `http://usoptiontown.com:5001/post/addcomment/${id}`,
        value
      );
